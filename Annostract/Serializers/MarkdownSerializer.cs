...
namespace Annostract.Serializer {
    public class MarkdownSerializer
    {
        ...
        public Task Serialize(IEnumerable<ExtractedFolder> folders, AnnostractSettings settings)
        {
            var res = "# Annostract Annotations";
            res += folders.Select(i => Serialize(i, 1)).CombineWithNewLine();
            res += "\n";
        ...
        public virtual string Serialize(ExtractedFolder folder, int headerLevel)
        {
            var res = $"## {folder.Name}\n\n";
            res += folder.Files.Select(i => Serialize(i, headerLevel+1)).CombineWithNewLine();
            res += folder.Folders.Select(i => Serialize(i, headerLevel+1)).CombineWithNewLine();
            res += "\n";
            return res; 
        }
